...
def Main(devices):
    class TC103(unittest.TestCase):
        ...
        def test_02_of_103(self):
            u'''用例test_02_of_103的操作步骤'''
            #每个函数里分别实例poco，否则容易出现pocoserver无限重启的情况
            print("我是TC102的test_02_of_103方法")
            poco = UnityPoco()
            Screencap.GetScreen(time.time(), devices, "test_02_of_103的描述")
            t = 1
    ...
    srcSuite = unittest.makeSuite(TC103)
    return srcSuite